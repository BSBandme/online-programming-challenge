...
int getuturn(int row, int col, int stindex, int dir) {
	...
	if((lstr - stindex) % 2 == 0) {
		ans += int(getmatch(row, col, stindex, dir) >= (lstr - stindex) / 2) *
				int(getmatch(1 - row, col + add * ((lstr - stindex) / 2 - 1), stindex + (lstr - stindex) / 2, 1 - dir) >= (lstr - stindex) / 2);
	}
//	ans += getuturn(row, col + add, stindex + 1, dir);
	return ans;
}
...
//............................不要再忘了检查maxn大小了！！！！BSBandme你个SB！！！！...................................................
	...
	if(lstr == 1) {
		int rans = 0;
		for(int i = 0; i < 2; i++)
			for(int j = 0; j < n; j++) if(mp[i][j] == str[0])
				rans++;
		cout << rans << endl;
		return 0;
	}
