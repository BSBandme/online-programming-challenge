...
const int mod = MOD;
const int maxn1 = 5010;
ll dp[maxn1][maxn1];
int nxt0[maxn1];
int arr[maxn], n, sa[maxn], rk[maxn], h[maxn];
char str[maxn1];
...
//............................不要再忘了检查maxn大小了！！！！BSBandme你个SB！！！！...................................................
	...
	for(int i = 0; i < n; i++) {
		for(int j = i; j >= 0; j--) {
			...
			if((i - (j - 1)) * 2 > i + 1) {
				if(j > 0) dp[j][i] = dp[0][j - 1];
				continue;
			}
//			if(j == 4 && i == 5) {
//				cerr << 1 << endl;
//			}
			int l = quelcp1(rk[j - 1 - (i - j)], rk[j]);
			bool flag = 1;
			if(l + j - 1 >= i || arr[j - 1 - (i - j) + l] > arr[j + l])
				flag = 0;
			if(flag)
				dp[j][i] = dp[j - 1 - (i - j)][j - 1];
			else
				dp[j][i] = dp[j - (i - j)][j - 1];
		}
		dp[0][i]++;
